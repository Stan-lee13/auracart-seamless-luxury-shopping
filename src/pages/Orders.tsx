  ...
  async function load() {
    ...
    try {
      const { data, error } = await supabase
        .from('orders')
        .select('id, order_number, grand_total, status, created_at, currency')
        .eq('user_id', user.id)
      ...
      {orders.length === 0 ? (
        <Card className="glass-card">
          <CardContent className="py-16 text-center">
            ...
            <Button onClick={() => navigate('/shop')} className="btn-luxury">Browse Products</Button>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardContent className="p-6">
                <div className="flex flex-col sm:flex-row justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <p className="price-display text-xl">₦{Number(order.grand_total || 0).toLocaleString()}</p>
                    <p className="text-sm text-muted-foreground">
                      {order.created_at ? new Date(order.created_at).toLocaleDateString('en-NG', { year: 'numeric', month: 'long', day: 'numeric' }) : '-'}
                    </p>
                  </div>
